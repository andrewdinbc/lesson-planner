...
//   merge       -- literal 2048-style merge board (Duck Duck Merge's core
//                  mechanic), with curriculum questions as a periodic
//                  bonus-tile trigger rather than gating every move --
//                  gating every single swipe behind a question would break
//                  the swipe-merge flow that makes the genre fun, so
//                  correct answers instead grant a bonus tile every few
//                  moves, keeping both halves (merging + content) intact
//   tycoon      -- correct-answers-earn-currency shop upgrade loop (Duck
//                  Duck Merge's "level up to make the most money" / Monkey
//                  Mart & My Perfect Hotel's "level up the store" feel --
//                  IMPORTANT distinction from the idle-clicker critique
//                  below: currency here is earned ONLY by answering
//                  correctly, never by idle ticking, so progress stays
//                  tied to engagement rather than replacing it)
//
...
// projected group event; muncher/fact_dash/merge/tycoon are solo-only for now.
