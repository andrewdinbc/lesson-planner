...
// ── Storage (service role — direct REST, no SDK) ──
// Uploads raw file bytes to a Supabase Storage bucket and returns the
// public URL. Used so uploaded PDFs are preserved as real files (not just
// extracted text) -- e.g. forge_resources.file_url.
export async function sbUploadFile(bucket, path, buffer, contentType) {
  ...
  if (!res.ok) throw new Error(`Supabase storage upload to ${bucket}/${path} failed: ${res.status} ${await res.text()}`)
