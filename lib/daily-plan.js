...
/**
 * Which block is "now", for the live desk-board view. Returns the block
 * whose [start_time, start_time+length) window contains the current
 * clock time, or null if the day hasn't started / has ended / is between
 * blocks (a gap). nowHHMM defaults to the real current time but takes an
 * override for testing.
 */
export function currentBlockId(blocks, nowHHMM) {
  const now = toMinutes(nowHHMM || new Date().toTimeString().slice(0, 5))
  for (const b of blocks) {
    const start = toMinutes(b.start_time)
    const end = start + b.length_minutes
...
/**
 * Given the Timeline's blocks (lib/timeline.js's timeline_units rows) and
 * the current instructional week number, find which unit is active per
 * subject this week. Used to auto-fill a day block's content with real
 * curriculum content instead of blank -- this is the actual Day <- Week
 * <- Year connective tissue Aj described: the day board should reflect
 * what the Year Timeline says is happening this week, not just a bare
 * time grid.
 */
export function activeUnitForSubjectThisWeek(timelineBlocks, subject, weekNumber) {
  if (!weekNumber) return null
  return timelineBlocks.find((b) => b.subject === subject && weekNumber >= b.start_week && weekNumber <= b.end_week) || null
}
