...
/**
 * Reorder units within a single subject (teaching sequence, distinct from
 * the priority slider's relative-weight concept -- sort_order is "what
 * order do I teach these in", priority is "how much time does each get").
 * Returns a NEW array covering just that subject's rows, re-numbered
 * 0..n-1 in the new order, ready to merge back into the full row set and
 * persist. Drag-and-drop on the Units page calls this on drop.
 */
export function reorderWithinSubject(subjectRows, fromIndex, toIndex) {
  const sorted = [...subjectRows].sort((a, b) => a.sort_order - b.sort_order)
  const [moved] = sorted.splice(fromIndex, 1)
  sorted.splice(toIndex, 0, moved)
  return sorted.map((r, i) => ({ ...r, sort_order: i }))
}
