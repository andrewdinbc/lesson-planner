...
// tone/structure only, see extract-style-pattern) and a GRADE the end user
// (whoever is designing or buying the product) selects, generates wholly
// original instructional content -- questions, passages, problems,
// prompts, tasks, examples -- grounded in:
//   1. The official BC curriculum content for that exact grade (public
//      Ministry material, curriculum-full-elaborations.js / bc-curriculum),
...
// grade the end user picked. This is what makes the resulting product
// legitimately sellable: the content is originated here, not extracted
// from anyone else's paid material. Aj, 2026-07-18.
...
export async function POST(request) {
  ...
  try {
    const { styleProfileId, subject, grade, topic } = await request.json()
    ...
    const curriculumBlock = curriculumGrade
...
Content: ${curriculumGrade.content.join(' | ')}`
      : `No structured curriculum data found for ${subject} Grade ${grade} -- use general grade-appropriate BC curriculum knowledge.`
    ...
    const prompt = `You are writing WHOLLY ORIGINAL instructional content for a BC teaching resource. This content must be entirely your own creation -- do not reference or reproduce any existing published resource's specific questions, passages, or exercises.
...
Write original instructional content appropriate for this grade and curriculum: 4-6 concrete items (questions, problems, prompts, or tasks depending on subject) that a teacher could use directly. Make it genuinely new material, grade-accurate, and written in the requested style.
...
{"title": "a title for this content set", "items": [{"type": "question|problem|prompt|task", "text": "the actual original content item"}]}`
    ...
    const [saved] = await sbInsert('forge_resources', [{
      ...
      title: generated.title || `Original content — ${subject} Grade ${grade}`,
      original_text: bodyText,
      edited_text: bodyText,
      status: 'edited',
    }])

    return Response.json({ content: generated, savedResourceId: saved.id })
  } catch (e) {
