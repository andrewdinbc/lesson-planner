// app/api/draw-sessions/[id]/join/route.js
...
export async function POST(request, { params }) {
  try {
    ...
    const [session] = await sbSelect('draw_sessions', `?id=eq.${id}&select=id,status&limit=1`)
    ...
    const [player] = await sbInsert('draw_players', [{ session_id: id, nickname: nickname.trim().slice(0, 24), score: 0 }])
